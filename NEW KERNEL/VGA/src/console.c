...
void console_clear(VGA_COLOR_TYPE fore_color, VGA_COLOR_TYPE back_color) {
    ...
    for (i = 0; i < VGA_TEXT_TOTAL_ITEMS; i++) {
        g_vga_buffer[i] = vga_text_item_entry(NULL, fore_color, back_color);
    }
    g_vga_index = 0;
    cursor_pos_x = 0;
    cursor_pos_y = 0;
...
void console_init(VGA_COLOR_TYPE fore_color, VGA_COLOR_TYPE back_color) {
    g_vga_buffer = (uint16 *)VGA_TEXT_ADDRESS;
...
static void console_newline() {
    if (cursor_pos_y >= VGA_TEXT_HEIGHT) {
        ...
        g_vga_index += VGA_TEXT_WIDTH - (g_vga_index % VGA_TEXT_WIDTH);
        cursor_pos_x = 0;
        ++cursor_pos_y;
        vga_text_set_cursor_pos(cursor_pos_x, cursor_pos_y);
    }
...
void console_putchar(char ch) {
    if (ch == ' ') {
        g_vga_buffer[g_vga_index++] = vga_text_item_entry(' ', g_fore_color, g_back_color);
        vga_text_set_cursor_pos(cursor_pos_x++, cursor_pos_y);
    }
    if (ch == '\t') {
        for(int i = 0; i < 4; i++) {
        ...
        if (ch > 0) {
            g_vga_buffer[g_vga_index++] = vga_text_item_entry(ch, g_fore_color, g_back_color);
            vga_text_set_cursor_pos(++cursor_pos_x, cursor_pos_y);
        }
    }
}
...
void console_ungetchar() {
    if(g_vga_index > 0) {
        g_vga_buffer[g_vga_index--] = vga_text_item_entry(0, g_fore_color, g_back_color);
        if(cursor_pos_x > 0) {
            vga_text_set_cursor_pos(cursor_pos_x--, cursor_pos_y);
        } else {
...
void console_ungetchar_bound(uint8 n) {
    if(((g_vga_index % VGA_TEXT_WIDTH) > n) && (n > 0)) {
        g_vga_buffer[g_vga_index--] = vga_text_item_entry(0, g_fore_color, g_back_color);
        if(cursor_pos_x >= n) {
            vga_text_set_cursor_pos(cursor_pos_x--, cursor_pos_y);
        } else {
